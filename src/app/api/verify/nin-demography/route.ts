...
export async function POST(req: NextRequest) {
  ...
  const { error: deductError } = await serviceSupabase
    .rpc('safe_deduct_credits', {
      ...
      p_reference: `DEMO_${user.id}_${parsed.data.firstname}_${parsed.data.lastname}_${parsed.data.gender}_${parsed.data.dob}`,
    })
  if (deductError) {
  ...
  if (result.status === 'error' || result.status === 'failed' || result.status === 'false') {
    await serviceSupabase.rpc('refund_wallet', {
      p_user_id: user.id,
      p_amount: COST,
      p_reference: `REFUND_DEMO_${user.id}_${parsed.data.firstname}_${parsed.data.lastname}_${parsed.data.gender}_${parsed.data.dob}`,
  ...
  if (insertError) {
    logger.error('Failed to insert api_call record for NIN demography', insertError)
  }
