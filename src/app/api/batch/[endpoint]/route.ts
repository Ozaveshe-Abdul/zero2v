...
export async function POST(req: NextRequest, { params }: { params: Promise<{ endpoint: string }> }) {
  try {
    ...
    const batchRef = `BATCH_${Date.now()}_${user.id.slice(0, 8)}`
    ...
    if (batchError || !batchJob) {
      ...
      await serviceSupabase.rpc('refund_wallet', {
        p_user_id: user.id,
        p_amount: totalCost,
        p_reference: `REFUND_BATCH_INIT_${batchRef}`,
    ...
    logger.error('Batch initiate error', err)
  ...
  try {
    ...
    for (const [index, payload] of items.entries()) {
      ...
      try {
        ...
        if (isError) {
          // Refund single item (deterministic ref — idempotent)
          await serviceSupabase.rpc('refund_wallet', {
            p_user_id: userId,
            p_amount: unitCost,
        ...
        // Isolated item error — refund and continue; don't crash the entire batch
        logger.error(`Batch item ${index + 1} failed for batch ${batchId}`, itemError)
        failedCount++
        ...
        await serviceSupabase.rpc('refund_wallet', {
          ...
          p_amount: unitCost,
          p_reference: `REF_BATCH_${batchId}_${index}`,
          p_description: `Batch refund: item ${index + 1} errored`,
        })
        ...
        await serviceSupabase.from('api_calls').insert({
          ...
          endpoint,
          request_payload: { ...payload, consent_given: true, batch_index: index },
          response_data: { status: 'error', message: getErrorMessage(itemError) },
          cost: 0,
          status: 'refunded',
          batch_id: batchId,
        })
    ...
    logger.error(`Batch processing crashed for ID ${batchId}`, error)

    // Refund credits for remaining unprocessed items
    ...
    const processed = (job?.completed_items ?? completedCount) + (job?.failed_items ?? failedCount)
    const unprocessed = items.length - processed
    if (unprocessed > 0) {
      const refundAmount = unprocessed * unitCost
      await serviceSupabase.rpc('refund_wallet', {
        p_user_id: userId,
        p_amount: refundAmount,
        p_reference: `REF_BATCH_CRASH_${batchId}`,
        p_description: `Batch crash refund: ${unprocessed} unprocessed item(s)`,
      })
    }
    ...
    await auditLog({
      ...
      severity: 'critical',
      userId,
      metadata: { batch_id: batchId, error: String(error), unprocessed_refunded: unprocessed },
    })
  }
}
