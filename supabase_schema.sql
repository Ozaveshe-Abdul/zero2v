...
begin
  ...
  -- Idempotency: if a debit with this reference already exists, skip
...
-- Idempotent Credit for Paystack webhooks (safe for duplicate delivery)
...
begin
  ...
  -- Idempotency: if a credit with this reference already exists, skip
...
create view orphaned_debits as
select
